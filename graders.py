...
def to_strict_unit_interval(value: float) -> float:
    """
    Ensure every score stays strictly inside (0,1)
    Never allow exact 0 or exact 1
    """
...
def choose_rule_based_action(obs: Dict) -> Dict:
    ...
    # Otherwise restock top-priority order
    target = sorted_orders[0]
    item = target["item"]
    need = int(target["quantity"])
    have = int(inventory.get(item, 0))
...
def grade_medium(result: Dict) -> float:
    ...
    first_fulfill_idx = next(
        (i for i, a in enumerate(actions) if a.get("action_type") == "fulfill"),
        None,
    )

    first_restock_idx = next(
        (i for i, a in enumerate(actions) if a.get("action_type") == "restock"),
        None,
    )

    restock_before_fulfill = (
        first_restock_idx is not None and
        (first_fulfill_idx is None or first_restock_idx < first_fulfill_idx)
    )
